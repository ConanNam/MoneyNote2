...
const caculateData = (dataS, dataE) => {
  let arrTitle = [];
  let dataAfter = [];
  ...
  for (let i = 0; i < arrTitle.length; i++) {
    ...
    if (tempE.length > 0 || tempS.length > 0) {
      dataAfter.push({
        title: title,
        dataItem: [tempS, tempE],
      });
    }
  }
  return dataAfter;
...
const OverView = ({navigation, route}) => {
  ...
  if (dataE?.length > 0 || dataS?.length > 0) {
    dataAll = caculateData(dataS, dataE);
            ...
            dataAll.map(item => {
              ...
              return (
                <View
                  ...
                  key={item.title}>
                  <View>
                    <Text style={{fontSize: 20}}>{fomatDate(item.title)}</Text>
                  </View>
                  <View
                    style={{
                      justifyContent: 'space-between',
